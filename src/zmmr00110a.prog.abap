*&---------------------------------------------------------------------*
*& Include  ZMMR00110A — GI 반품 사유 분석 ALV Field Catalog
*&---------------------------------------------------------------------*

*----------------------------------------------------------------------
" convert_fcat_data_grid — screen-independent utility FORM
" Builds LVC_T_FCAT from SALV factory for the given table reference.
*----------------------------------------------------------------------
...
*----------------------------------------------------------------------
" modify_fcat_data_grid1_0100 — adjust ALV column attributes for screen 0100
*----------------------------------------------------------------------
FORM modify_fcat_data_grid1_0100 CHANGING pt_fieldcat TYPE lvc_t_fcat.
  ...
  LOOP AT pt_fieldcat ASSIGNING <fs_fcat>.
    CASE <fs_fcat>-fieldname.
      ...
      WHEN 'LGORT'.    <fs_fcat>-coltext = text-f07. <fs_fcat>-outputlen = 4.
      WHEN 'LIFNR'.    <fs_fcat>-coltext = text-f08. <fs_fcat>-outputlen = 10.
      WHEN 'RETURN_QTY'. <fs_fcat>-coltext = text-f09. <fs_fcat>-qfieldname = 'MEINS'. <fs_fcat>-do_sum = abap_true.
      WHEN 'MEINS'.    <fs_fcat>-coltext = text-f10. <fs_fcat>-no_out = abap_true.
      WHEN 'REASON_CODE'. <fs_fcat>-coltext = text-f11. <fs_fcat>-outputlen = 4.
      WHEN 'VENDOR_FAULT'. <fs_fcat>-coltext = text-f12. <fs_fcat>-outputlen = 1.
      WHEN 'CLAIM_AMOUNT'. <fs_fcat>-coltext = text-f13. <fs_fcat>-cfieldname = 'WAERS'. <fs_fcat>-do_sum = abap_true.
      WHEN 'WAERS'.    <fs_fcat>-coltext = text-f14. <fs_fcat>-no_out = abap_true.
      WHEN 'RETURN_DATE'. <fs_fcat>-coltext = text-f15. <fs_fcat>-outputlen = 10.
    ENDCASE.
  ENDLOOP.
ENDFORM.
